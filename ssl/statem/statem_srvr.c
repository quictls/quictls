...
static CON_FUNC_RETURN tls_construct_encrypted_extensions(SSL *s,
                                                          WPACKET *pkt);

static inline int received_client_cert(const SSL *s)
{
    return s->session->peer_rpk != NULL || s->session->peer != NULL;
}
 ...
 */
static int get_compressed_certificate_alg(SSL *sc)
{
#ifndef OPENSSL_NO_COMP_ALG
...
static int do_compressed_cert(SSL *sc)
{
...
CON_FUNC_RETURN dtls_construct_hello_verify_request(SSL *s,
                                                    WPACKET *pkt)
{
    ...
    if (sctx->app_gen_cookie_cb == NULL
        || sctx->app_gen_cookie_cb(s, s->d1->cookie,
                                   &cookie_leni) == 0
 ...
 */
static void ssl_check_for_safari(SSL *s,
...
static int tls_early_post_process_client_hello(SSL *s)
{
    ...
    if (sctx->client_hello_cb != NULL) {
        ...
        switch (sctx->client_hello_cb(s, &al, sctx->client_hello_cb_arg)) {
    ...
    if (SSL_IS_DTLS(s)) {
        ...
        if (SSL_get_options(s) & SSL_OP_COOKIE_EXCHANGE) {
            if (sctx->app_verify_cookie_cb != NULL) {
                if (sctx->app_verify_cookie_cb(s, clienthello->dtls_cookie,
...
static int tls_handle_status_request(SSL *s)
{
    ...
    if (s->ext.status_type != TLSEXT_STATUSTYPE_nothing && sctx != NULL
        ...
        if (s->s3.tmp.cert != NULL) {
            ...
            ret = sctx->ext.status_cb(s, sctx->ext.status_arg);
            switch (ret) {
...
int tls_handle_alpn(SSL *s)
{
    ...
    if (sctx->ext.alpn_select_cb != NULL && s->s3.alpn_proposed != NULL) {
        int r = sctx->ext.alpn_select_cb(s,
...
CON_FUNC_RETURN tls_construct_server_hello(SSL *s, WPACKET *pkt)
{
    int compm;
    size_t sl, len;
    int version;
    ...
    if (!WPACKET_sub_memcpy_u8(pkt, session_id, sl)
            || !s->method->put_cipher_by_char(s->s3.tmp.new_cipher,
                                                                      pkt, &len)
...
CON_FUNC_RETURN tls_construct_server_done(SSL *s, WPACKET *pkt)
{
    if (!s->s3.tmp.cert_request) {
...
CON_FUNC_RETURN tls_construct_server_key_exchange(SSL *s,
                                                  WPACKET *pkt)
{
    EVP_PKEY *pkdh = NULL;
    ...
    if (type & (SSL_kDHE | SSL_kDHEPSK)) {
        ...
        if ((pkdhp == NULL) && (s->cert->dh_tmp_cb != NULL)) {
            pkdh = ssl_dh_to_pkey(s->cert->dh_tmp_cb(s,
                                                     0, 1024));
            if (pkdh == NULL) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                goto err;
...
CON_FUNC_RETURN tls_construct_certificate_request(SSL *s,
                                                  WPACKET *pkt)
{
    if (SSL_IS_TLS13(s)) {
...
static int tls_process_cke_psk_preamble(SSL *s, PACKET *pkt)
{
    ...
    psklen = s->psk_server_callback(s,
...
MSG_PROCESS_RETURN tls_process_client_rpk(SSL *sc, PACKET *pkt)
{
...
#ifndef OPENSSL_NO_COMP_ALG
MSG_PROCESS_RETURN tls_process_client_compressed_certificate(SSL *sc, PACKET *pkt)
{
    MSG_PROCESS_RETURN ret = MSG_PROCESS_ERROR;
    PACKET tmppkt;
...
CON_FUNC_RETURN tls_construct_server_certificate(SSL *s, WPACKET *pkt)
{
...
#ifndef OPENSSL_NO_COMP_ALG
CON_FUNC_RETURN tls_construct_server_compressed_certificate(SSL *sc, WPACKET *pkt)
{
...
static int create_ticket_prequel(SSL *s, WPACKET *pkt,
...
static CON_FUNC_RETURN construct_stateless_ticket(SSL *s,
                                                  WPACKET *pkt,
                                                  uint32_t age_add,
...
static int construct_stateful_ticket(SSL *s, WPACKET *pkt,
                                     uint32_t age_add,
                                     unsigned char *tick_nonce)
{
    if (!create_ticket_prequel(s, pkt, age_add, tick_nonce)) {
...
CON_FUNC_RETURN tls_construct_new_session_ticket(SSL *s, WPACKET *pkt)
{
    SSL_CTX *tctx = s->session_ctx;
...
CON_FUNC_RETURN tls_construct_cert_status(SSL *s, WPACKET *pkt)
{
