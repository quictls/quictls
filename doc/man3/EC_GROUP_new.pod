...
The sole supported curves are those defined over a prime field Fp. The elements of
Fp are the integers 0 to p-1, where p is a prime number. This gives us a revised
...
implementation provided by I<meth> ) and
...
over Fp I<p> is the prime for the field.

EC_group_get_curve() obtains the previously set curve parameters.

EC_GROUP_set_curve_GFp() is an alias for
EC_GROUP_set_curve(). It is defined for backwards compatibility only and
should not be used.

EC_GROUP_get_curve_GFp() is an alias for
EC_GROUP_get_curve(). It is defined for backwards compatibility only and
should not be used.

The functions EC_GROUP_new_curve_GFp() is a shortcut for calling EC_GROUP_new()
and then the EC_GROUP_set_curve() function. An appropriate default implementation
method will be used.
...
EC_GROUP_set_curve_GFp(), EC_GROUP_get_curve_GFp() return 1 on success or 0 on error.
...
=item *
 ...
 EC_GROUP_set_curve_GFp(), EC_GROUP_get_curve_GFp(), were deprecated in
