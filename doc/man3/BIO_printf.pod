...
BIO_printf, BIO_vprintf, BIO_vsnprintf
...
BIO_vsnprintf() is like the standard C snprintf() function, and is
maintained for compatibility with cryptographic providers that need
this upcall.
...
For BIO_vsnprintf() this includes when the output
buffer is too small.

=head1 NOTES

Except when I<n> is 0, BIO_vsnprintf() always
terminates its output with C<'\0'>.  This includes cases where -1 is
returned, such as when there is insufficient space to generate the whole
string.
