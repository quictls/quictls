...
prohibits them (for example SHA-1 if the security level is 4 or more).
...
example B<MD5>, B<SHA1>, B<SHA224>, B<SHA256>, B<SHA384>, B<SHA512>) and
the public key algorithm strings B<RSA>, B<RSA-PSS>, B<DSA> or B<ECDSA>.
...
Set supported signature algorithms to SHA-256 with ECDSA and SHA-256 with RSA
using an array:
