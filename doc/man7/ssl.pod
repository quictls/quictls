...
TLSv1.1, TLSv1.2 and TLSv1.3) and DTLS (DTLSv1 and DTLSv1.2).
The library depends on C<libcrypto> for its underlying cryptographic
operations (see L<crypto(7)>).
...
TLS supports the concept of a "stream" of data. Data sent via a
...
SSL/TLS or DTLS. It is passed as a parameter when creating
the B<SSL_CTX>.
