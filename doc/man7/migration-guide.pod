...
The provider interface is compatible with the OpenSSL 3.0 release.
To meet FIPS requirements, you should use the provider for OpenSSL.
...
Check the various C<NOTES> and C<README> files.
...
EC_GROUP_get_curve_GFp(), EC_GROUP_set_curve_GFp()
...
EC_POINT_get_affine_coordinates_GFp(), EC_POINT_set_affine_coordinates_GFp()
...
SHA-256 is now the default digest for TS query used by B<openssl ts>.
...
using third party provider groups may result in handshake failures.
...
The security strength of SHA-1 and MD5 based signatures in TLS has been reduced.
...
set requires the availability of SHA-1.

=item *

X509 certificates signed using SHA-1 are no longer allowed at security level 1 and above.
