...
These options are outdated and are ignored since the entropy source
for the random-number generators is taken from the local platform.
...
about specific commands, see L<openssl-engine(1)> and
L<openssl-rehash(1)>.
