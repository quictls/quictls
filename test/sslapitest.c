                                            ...
                                            size_t secret_len)
{
    test_printf_stderr("quic_set_encryption_secrets() %s, lvl=%d, len=%zd\n",
                       SSL_is_server(ssl) ? "server" : "client", level, secret_len);
    return 1;
}
                                        ...
                                        const uint8_t *data, size_t len)
{
    ...
    TEST_info("quic_add_handshake_data() %s, lvl=%d, *data=0x%02X, len=%zd\n",
              SSL_is_server(ssl) ? "server" : "client", level, (int)*data, len);
    if (!TEST_ptr(peer))
        return 0;
...
static int test_quic_send_alert(SSL *ssl, enum ssl_encryption_level_t level, uint8_t alert)
{
    test_printf_stderr("quic_send_alert() %s, lvl=%d, alert=%d\n",
                       SSL_is_server(ssl) ? "server" : "client", level, alert);
    return 1;
}
...
OPT_TEST_DECLARE_USAGE("certfile privkeyfile DUMMY tmpfile provider config dhfile\n")
